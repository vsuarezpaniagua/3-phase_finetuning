...
class VAELoss(nn.Module):
    """
        ...
        Here the argument 'decoder_name_or_path' indicates the pretrained model (supported by Huggingface)
        to be used as the decoder. Since decoding process is included,
        here the decoder should have a class called XXXLMHead (in the context of Huggingface's Transformers).
    ...
    def __init__(
        ...
        :param decoder_name_or_path: Model name or path for initializing a decoder
            (compatible with Huggingface's Transformers)
        ...
        try:
            ...
            logger.error(f'Model name or path "{decoder_name_or_path}" does not support being as a decoder.'
                         f'Please make sure the decoder model has an "XXXLMHead" class.')
            raise e

        enc_out_dim = model[0].auto_model.config.hidden_size
        # dec_input_dim = decoder_config.hidden_size
        ...
        if len(AutoTokenizer.from_pretrained(encoder_name_or_path)) != len(self.tokenizer_encoder):
            logger.warning('WARNING: The vocabulary of the encoder has been changed.'
                           'One might need to change the decoder vocabulary, too.')
        ...
        if tie_encoder_decoder:
            ...
            if len(self.tokenizer_encoder) != len(self.tokenizer_decoder):  # The vocabulary has been changed.
                ...
                logger.warning(
                    'Since the encoder vocabulary has been changed and --tie_encoder_decoder=True, '
                    'now the new vocabulary has also been used for the decoder.')
    ...
    def likelihood(self, x_hat, logscale, x):
        # gaussian_likelihood
    ...
    def forward(self, sentence_features: Iterable[Dict[str, Tensor]], labels: Tensor):
        ...
        if self.need_retokenization:
            ...
            # re-tokenization by the decoder's one is needed if different tokenizers used
        ...
        mu, log_var = self.fc_mu(reps), self.fc_var(reps)
        ...
        x_ids = target_features['input_ids'][:, 1:]
        ...
        decoder_outputs = self.decoder(
            ...
            inputs_embeds=None,
            attention_mask=None,
            encoder_hidden_states=z[:, None],  # (bsz, hdim) -> (bsz, 1, hdim)
        ...
        # reconstruction loss
        recon_loss = self.likelihood(lm_logits.view(-1, lm_logits.shape[-1]),
                                     self.log_scale, x_ids.reshape(-1))
