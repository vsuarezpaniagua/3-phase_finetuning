...
# Monkey patch the requests functions
from src.training_functions import check_dtype
...
def multiple_training_CLFT(encoder, dataset, encoder_path, HYPER,
    ...
    extra_name = "_LR" + str(PHASE1_LR) + str(PHASE2_LR) + str(PHASE3_LR) + "_EP" + str(EPOCHS_PHASE1) + str(
        EPOCHS_PHASE2) \
                 + str(PHASE3_EPOCHS_MAX) + "_BA" + str(PHASE1_BATCH) + str(PHASE2_BATCH) + str(PHASE3_BATCH) \
                 + str(PHASE3_eps) + str(PHASE2_AUGMENTATION) \
                 + "U" + str(1 if use_max_len else 0) + "G" + str(1 if given_max_len else 0) \
                 + "F" + str(1 if freezing else 0) + "D" + str(del_ratio)
    ...
    # base_architecture_type = "transformers"

    final_2 = training_sbert(join(PHASE2_TRAIN, "PHASE2_ablation_CL_FT" + unbalance + model_name),
                             evaluation_metric,
                             level,
                             model_type,
                             model_path,
    ...
    # final_2 = join(PHASE2_TRAIN, "PHASE2_" + model_name)
    ...
    train_loss, metric_cl_ablation = trainer(PHASE3_EPOCHS_MIN, PHASE3_EPOCHS_MAX, train_dataloader,
                                             test_dataloader, model, optimizer, device, scheduler,
                                             model_type, average)

    final_3 = join(PHASE3_TRAIN, "PHASE3_cl_ft_" + unbalance + model_name)
    ...
    save_and_plot(metric_cl_ablation, "main_ablation_cl_ft_" + unbalance, save_name)
    for dataset in ["SST2"]:
...
def multiple_training_DAEFT(encoder, dataset, encoder_path, HYPER,
                            list_extra_names=[], test_mode=False, long_mode=False):
    """ One of the iterations in the loop for one of combination of hyperparameters.
    Input:
        - encoder (tuple): (models path: str, model type: str, lengths: list[str])
        - dataset (tuple):
    ...
    # ############ Training phase 1
    final = dae_trainer(join(PHASE1_TRAIN, "PHASE1_ablation_DAE_FT" + model_name),
                        data_path_phase1,
                        data_path_phase2,
                        evaluate,
                        model_type,
    ...
    final_3 = join(PHASE3_TRAIN, "PHASE3_DAE_FT" + model_name)
    print("phase 3 checkpoint path", final_3)
    try:
        mkdir(final_3)
    except OSError as error:
    ...
    save_and_plot(metric_dict_main, "ablation_DAE_FT", save_name)
